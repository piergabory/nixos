  ...
  config = mkIf cfg.enable {
    services.authelia.instances.main = {
      ...
      secrets = with config.age.secrets; {
        ...
        sessionSecretFile = session.path;
        storageEncryptionKeyFile = storage-key.path;
        oidcHmacSecretFile = oidc-hmac.path;
        oidcIssuerPrivateKeyFile = oidc-jwks.path;
      };
      ...
      settingsFiles = [ config.age.secrets.oidc-clients.path ];
      ...
      settings = {
        ...
        authentication_backend = {
          ...
          file = {
            path = config.age.secrets.users.path;
            watch = false;
