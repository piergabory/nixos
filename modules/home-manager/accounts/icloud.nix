...
in
{
  ...
  options.accounts.icloud = {
    ...
    appSpecificPasswordFile = mkOption {
      type = types.str;
    };
    realName = mkOption {
      type = types.str;
  ...
  config = mkIf cfg.enable {
    config.age.secrets = {
      icloud-mail.file = ../../../../secrets/icloud-mail.age;
      icloud-dav.file = ../../../../secrets/icloud-dav.age;
    };
    ...
    accounts = {
      email.accounts.icloud = {
        ...
        passwordCommand = ["cat" config.age.secrets.icloud-mail.path];
      };
    };
  } // mkDavAccountSync {
    userName = cfg.appleId;
